...
## learning "base" and “tidyverse” functions. 
### Base R is deployed with the core packages while tidyverse is
### a collection of packages that build on top of the core R to
### make easier handling data 
# credit to [tavareshugo.github.io](https://tavareshugo.github.io/data_carpentry_extras/)

> It is important to understand that there are many different ways to achieve 
the same thing, particularly in base R. I have tried to use 
simpler syntax where possible. For the more complicated cases, 
I have based my solutions on top hits from web searches.

It is also worth remembering that when you look for help online, you might 
find solutions using “base R” syntax. If you are used to “tidyverse” packages, 
it will help to add keywords such as “dplyr” and “tidyverse” to your search.

There are many other resources available online, in particular several 
cheatsheets that might be useful as a reference: 
...
The datasets used in these examples come pre-loaded with R. 
To find more about them check their help pages:
...
```{r}
selection <- select(iris, Species, Petal.Width) # by name
head(selection)
selection <- select(iris, 5, 4)  # by column index
head(selection)
```
...
```{r}
selection <- iris[, c("Species", "Petal.Width")] # by name
head(selection)
selection <- iris[, c(5, 4)]  # by column index
head(selection)
```
...
```{r}
mutated <- iris$Petal.Ratio <- iris$Petal.Length/iris$Petal.Width
mutated <- iris$Sepal.Ratio <- iris$Sepal.Length/iris$Sepal.Width
head(mutated)
```
...
```{r}
filtered <- filter(iris, Petal.Width > 0.5 & Species == "setosa")
head(filtered)
```
...
```{r}
# Using [,]
filtered <- iris[iris$Petal.Width > 0.5 & iris$Species == "setosa", ]
head(filtered)
```

```{r}
# Using subset (works very much like dplyr::filter)
filtered <- subset(iris, Petal.Width > 0.5 & Species == "setosa")
head(filtered)
```
...
```{r}
inner_join(band_members, band_instruments2, 
           by = c("name" = "artist"))
```
...
> Important note: with dplyr, grouped operations are initiated 
with the function group_by(). It is a good habit to use ungroup() at the 
end of a series of grouped operations, otherwise the groupings will be 
carried in downstream analysis, which is not always desirable. 
In the examples below we follow this convention.

> Note on base R: with base R there are several ways to do these types of 
operations. Here we show a generic “split-apply-combine” solution using 
the by() function. These solutions require some understanding of list 
...
```{r}
gather(iris, key = "trait", value = "measurement", 
...
```{r}
stack(iris, select = c("Sepal.Length", "Sepal.Width", 
                       "Petal.Length", "Petal.Width"))
```
...
> Note that in this case it might have been better to first modify 
the “key” column so that the resulting column names are not numeric:
